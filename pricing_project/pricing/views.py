...
class CalculatePriceAPI(APIView):
    ...
    def post(self, request):
        ...
        # Find active config for this day (SQLite-safe filtering)
        configs = PricingConfig.objects.filter(is_active=True)
        config = next((c for c in configs if day in c.days_of_week), None)
        ...
        if waiting_time <= config.free_waiting_minutes:
            ...
            intervals = -(-chargeable_time // config.waiting_charge_interval)  # ceiling division
